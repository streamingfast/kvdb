...
func (db *DB) scanAccountRows(rows *sql.Rows) (out []*pbdeos.AccountCreationRef, err error) {
	for rows.Next() {
		account := &pbdeos.AccountCreationRef{
			Account:       "",
			Creator:       "",
			BlockNum:      0,
			BlockId:       "",
...
func (db *DB) putNewAccount(blk *pbdeos.Block, trace *pbdeos.TransactionTrace, act *pbdeos.ActionTrace) error {
